...
export default class BudgetingScr extends React.Component {
  ...
  loadWeeklyBudget() {
    getWeeklyBudget().then((weeklyBudget) => {
      console.log(
        "inside BudgetingScreen, printing weekly budget ",
        weeklyBudget
      );

      this.setState({ budgetValue: weeklyBudget });
    });
  }

  loadSpendingCategories() {
    getAllSpendingCategories().then((spendingCategoriesArray) => {
      this.setState({ categories: spendingCategoriesArray });
    });
  }

  componentDidMount() {
    // Load weekly budget from firestore
    this.loadWeeklyBudget();

    // Load existing spending categories from firestore
  ...
  saveBudgetValue = () => {
    this.setState({
      budgetValue: this.state.budgetInputValue
        ? this.state.budgetInputValue
        : 0,
      budgetInputValue: "",
    });
    ...
    editWeeklyBudget(this.state.budgetInputValue);
  };
  ...
  removeCategory = (i) => (e) => {
    ...
    deleteSpendingCategory(category).then(() => {
      console.log("inside BudgetingScr, category was successfully deleted");
    });
  };
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          <View style={styles.dashboardView}>
            ...
            <View style={styles.budgetView}>
              <Text style={styles.headerText}>
                Current Weekly Budget: ${budgetValue}
              </Text>
            </View>
            <View style={styles.budgetInputView}>
              <Text style={{ fontSize: 14 }}>
                Set a new weekly total budget amount:
              </Text>
              <TextInput
                theme={{ colors: { primary: "#5B5B5B" } }}
                style={styles.budgetInput}
                onChangeText={this.handleBudgetInput}
            ...
            <View style={styles.budgetView}>
              <Text style={{ fontSize: 14 }}>
                Add a new spending category (amount per week):
              </Text>
            </View>
            <View style={styles.budgetView}>
              <TextInput
                theme={{ colors: { primary: "#5B5B5B" } }}
                onChangeText={this.handleCategoryNameInput}
                value={curCatNameInput}
                style={styles.categoryNameInput}
                placeholder="Category Name"
              />
              <TextInput
                theme={{ colors: { primary: "#5B5B5B" } }}
                onChangeText={this.handleCategoryAmountInput}
                value={curCatAmntInput}
                style={styles.categoryAmntInput}
                placeholder="$$$$"
                keyboardType="numeric"
              />
              <Button
          ...
          >
            {categories.length == 0 ? (
              <Text
                style={{ fontSize: 16, fontWeight: "bold", color: "#9FB48F" }}
              >
                No Categories Added
              </Text>
            ) : (
                  ...
                  <DataTable.Row key={i}>
                    ...
                    <DataTable.Cell style={{ justifyContent: "flex-end" }}>
                      <Button
                        mode="contained"
                        compact={true}
                        style={{ borderWidth: 1, backgroundColor: "#9FB48F" }}
                        onPress={this.removeCategory(i)}
                        icon="close"
                      ></Button>
                    </DataTable.Cell>
                  </DataTable.Row>
          ...
          <View style={styles.expenseView}>
            {this.state.categories.length == 0 || (
              <Text style={styles.expenseText}>
                Amount of Budget Not Allocated: ${budgetValue - totalExpense}
              </Text>
            )}
          </View>
        </ScrollView>
